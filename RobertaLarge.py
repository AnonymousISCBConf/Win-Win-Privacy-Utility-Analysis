...
output_dir = "./roberta-large-result-double-win/"
...
def create_datasets(train_df, test_df):
    ...
    # RoBERTa uses byte-level BPE, so we don't need to add special nucleotide tokens
    # It can handle any character sequence naturally

    def tokenize_function(examples):
        # For DNA sequences, insert spaces between nucleotides for better tokenization
        tokenized = []
        for seq in examples['sequence']:
            # Insert spaces between nucleotides
...
# 3. Fine-tuning RoBERTa-large
def finetune_roberta(tokenized_train, tokenized_test, tokenizer):
    print("Starting fine-tuning of RoBERTa-large...")

    # Load pre-trained RoBERTa-large for sequence classification
    model = RobertaForSequenceClassification.from_pretrained(model_name, num_labels=2)

    # Define training arguments
    # RoBERTa-large has ~355M parameters (24 layers, 1024 hidden dimensions, 16 attention heads)
    training_args = TrainingArguments(
        output_dir=f"{output_dir}finetune",
        learning_rate=2e-5,  # Standard learning rate for large models
        per_device_train_batch_size=16,  # Smaller batch size due to larger model
    ...
    print(f"Fine-tuned RoBERTa-large evaluation: {eval_result}")

    # Save the model
    model.save_pretrained(f"{output_dir}finetune_roberta_large")
    tokenizer.save_pretrained(f"{output_dir}finetune_roberta_large")

    return model, eval_result

# 4. Extract CLS token embeddings from RoBERTa-large
def extract_embeddings(model, tokenizer, sequences):
    print(f"Extracting embeddings from RoBERTa-large model...")

    # Use the base RoBERTa model for feature extraction
    feature_extractor = model.roberta  # Note: RoBERTa uses 'roberta' not 'bert'
    ...
    with torch.no_grad():
        for i in range(0, len(sequences), batch_size):
            ...
            # Extract CLS token embeddings (first token, same as BERT)
            # RoBERTa uses <s> token instead of [CLS], but it's still the first token
            # For RoBERTa-large, this will be 1024-dimensional
            cls_token_embeddings = outputs[0][:, 0, :].cpu().numpy()
            all_embeddings.append(cls_token_embeddings)

    # Concatenate all batches
    embeddings = np.vstack(all_embeddings)
    print(f"Extracted embeddings shape: {embeddings.shape}")  # Should be (n_samples, 1024)
...
def train_mlp(train_embeddings, train_labels, test_embeddings, test_labels, model_type):
    ...
    # Initialize model
    input_dim = train_embeddings.shape[1]  # 1024 for RoBERTa-large
...
def create_comprehensive_visualizations(all_metrics):
    ...
    plt.title('RoBERTa-large Training Metrics by Epoch', fontsize=14)
    ...
    plt.title('RoBERTa-large Performance Metrics by Epoch', fontsize=14, pad=20)
    plt.tight_layout()
...
def run_reconstruction_attack(embeddings, sequences, model_name, output_dir="./roberta-large-result-double-win/"):
    ...
    embedding_dim = embeddings.shape[1]  # 1024 for RoBERTa-large
    seq_length = 20
...
def compare_reconstruction_attacks(pretrained_results, finetuned_results, output_dir="./roberta-large-result-double-win/"):
...
# 14. Calculate Pareto Dominance Score (Delta)
...
# 15. Calculate Pareto Dominance Score (Gain)
...
def visualize_privacy_metrics(delta_privacy, privacy_gain, pds_delta, pds_gain, utility_delta, output_dir):
    """Visualize the privacy metrics with all visualizations"""
...
def create_pds_comparison(pds_delta, pds_gain, delta_privacy, privacy_gain, output_dir):
    """Create enhanced visualizations comparing PDS metrics"""
...
def main():
    print("Starting RoBERTa-large fine-tuning, embedding extraction, and privacy analysis pipeline")
    ...
    # Load pretrained RoBERTa-large
    print("Loading pretrained RoBERTa-large model...")
    pretrained_model = RobertaForSequenceClassification.from_pretrained(model_name, num_labels=2)
    ...
    _, pretrained_metrics = train_mlp(
        ...
        model_type="pretrained_roberta_large"
    )
    ...
    # Fine-tune the model
    finetuned_model, roberta_eval_results = finetune_roberta(tokenized_train, tokenized_test, tokenizer)
    ...
    _, finetuned_metrics = train_mlp(
        ...
        model_type="finetuned_roberta_large"
    )
    ...
    plt.title('RoBERTa-large Fine-tuning Privacy-Utility Analysis: Summary Results', fontsize=14, pad=20)
    plt.tight_layout()
