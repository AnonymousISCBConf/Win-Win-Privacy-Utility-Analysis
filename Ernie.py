...
# Use ERNIE 2.0 base English model
model_name = "nghuyong/ernie-2.0-base-en"
output_dir = "./ernie-2.0-result-double-win/"
os.makedirs(output_dir, exist_ok=True)

print(f"Using ERNIE 2.0 model: {model_name}")
print(f"ERNIE 2.0 Architecture: 12 transformer layers, 768 hidden dimensions, 12 attention heads")
...
def create_datasets(train_df, test_df):
    ...
    # Add nucleotide tokens to tokenizer
    special_tokens = {'additional_special_tokens': ['A', 'C', 'G', 'T']}
    num_added = tokenizer.add_special_tokens(special_tokens)
    
    if num_added > 0:
        print(f"Added {num_added} special nucleotide tokens to ERNIE 2.0 tokenizer")
    ...
    print("Tokenizing datasets with ERNIE 2.0 tokenizer...")
...
# 3. Fine-tuning ERNIE 2.0
def finetune_ernie(tokenized_train, tokenized_test, tokenizer):
    print("Starting fine-tuning of ERNIE 2.0...")
    ...
    print("Training ERNIE 2.0 with knowledge-enhanced representations...")
    trainer.train()

    eval_result = trainer.evaluate()
    print(f"Fine-tuned ERNIE 2.0 evaluation: {eval_result}")

    model.save_pretrained(f"{output_dir}finetune_ernie_2.0")
    tokenizer.save_pretrained(f"{output_dir}finetune_ernie_2.0")

    return model, eval_result

# 4. Extract CLS token embeddings from ERNIE 2.0
def extract_embeddings(model, tokenizer, sequences):
    print(f"Extracting embeddings from ERNIE 2.0 model...")

    # Access base model
    if hasattr(model, 'ernie'):
        feature_extractor = model.ernie
    ...
    print(f"Extracted ERNIE 2.0 embeddings shape: {embeddings.shape}")

    return embeddings

# 5. Implement Pan's 3-layer MLP classifier
...
def create_comprehensive_visualizations(all_metrics):
    ...
    plt.title('ERNIE 2.0 Training Metrics by Epoch', fontsize=14)
    ...
    plt.title('ERNIE 2.0 Performance Metrics by Epoch', fontsize=14, pad=20)
    plt.tight_layout()
...
# 10. Create positional embeddings
...
# 11. Reconstruction attack implementation
def run_reconstruction_attack(embeddings, sequences, model_name, output_dir="./ernie-2.0-result-double-win/"):
...
def compare_reconstruction_attacks(pretrained_results, finetuned_results, output_dir="./ernie-2.0-result-double-win/"):
    ...
    plt.title('Comparison of Reconstruction Attack Accuracy: Pretrained vs. Fine-tuned ERNIE 2.0', fontsize=14)
    plt.xlabel('Position', fontsize=12)
    plt.ylabel('Accuracy', fontsize=12)
    plt.xticks(x_pos)
    plt.ylim(0, 1.05)
    ...
    ax.set_title('Nucleotide-wise Reconstruction Accuracy Comparison (ERNIE 2.0)')
    ax.set_xticks(x)
    ...
    plt.title('Privacy Change After Fine-tuning ERNIE 2.0 (Positive = More Private)', fontsize=14)
    plt.xlabel('Position', fontsize=12)
    ...
    plt.title('Reconstruction Attack Comparison Summary (ERNIE 2.0)', fontsize=14, pad=20)
    plt.tight_layout()
...
# 13. Calculate Error-Based Privacy Gain
...
# 16. Calculate Win-Win Percentage
...
def visualize_privacy_metrics(delta_privacy, privacy_gain, pds_delta, pds_gain, utility_delta, output_dir):
    ...
    summary_text = [
        "Privacy-Utility Analysis Summary (ERNIE 2.0)",
        "=============================================",
...
def create_position_pareto_frontiers(delta_privacy, privacy_gain, utility_delta, output_dir):
    ...
    plt.title('Position-Specific Analysis: Delta Privacy (ERNIE 2.0)', fontsize=14)
    ...
    plt.title('Position-Specific Analysis: Privacy Gain (ERNIE 2.0)', fontsize=14)
    plt.xlabel('Utility Gain (%)', fontsize=12)
    plt.ylabel('Privacy Gain (%)', fontsize=12)
    plt.grid(True, alpha=0.3)
    plt.legend(loc='best')
    ...
    plt.title('Position Rankings: Delta Privacy vs Privacy Gain (ERNIE 2.0)', fontsize=16, pad=20)
    plt.tight_layout()
    ...
    plt.title('Comparison of Win-Win Positions Between Metrics (ERNIE 2.0)', fontsize=14)
    plt.tight_layout()
...
def create_pds_comparison(pds_delta, pds_gain, delta_privacy, privacy_gain, output_dir):
    ...
    ax1.set_title('PDS-Delta: Based on Raw Accuracy Difference (ERNIE 2.0)', fontsize=14, fontweight='bold')
    ...
    ax2.set_title('PDS-Gain: Based on Logarithmic Error Reduction (ERNIE 2.0)', fontsize=14, fontweight='bold')
    ...
    plt.title('PDS Metrics Comparison Summary (ERNIE 2.0)', fontsize=14, pad=40)
    plt.tight_layout()
...
def main():
    print("=" * 80)
    print("Starting ERNIE 2.0 Fine-tuning, Embedding Extraction, and Privacy Analysis")
    print("=" * 80)
    print(f"Model: {model_name}")
    print(f"ERNIE 2.0 Features:")
    print("  - Enhanced Representation through kNowledge IntEgration")
    print("  - Continual pre-training framework with multi-task learning")
    print("  - Knowledge-enhanced masking strategies (entity-level & phrase-level)")
    print("  - 12 layers, 768 dimensions, 12 attention heads (110M parameters)")
    ...
    print("\n=== Training MLP on Pretrained ERNIE 2.0 Embeddings ===")
    _, pretrained_metrics = train_mlp(
        ...
        model_type="pretrained_ernie_2.0"
    )
    ...
    print(f"Saved pretrained ERNIE 2.0 embeddings to {output_dir}")

    finetuned_model, ernie_eval_results = finetune_ernie(tokenized_train, tokenized_test, tokenizer)

    print("\nExtracting embeddings from fine-tuned ERNIE 2.0 model...")
    ...
    print(f"Saved fine-tuned ERNIE 2.0 embeddings to {output_dir}")

    print("\n=== Training MLP on Fine-tuned ERNIE 2.0 Embeddings ===")
    _, finetuned_metrics = train_mlp(
        ...
        model_type="finetuned_ernie_2.0"
    )
    ...
    print(f"\nUtility delta (fine-tuned - pretrained): {utility_delta:.4f}")

    print("\n" + "=" * 80)
    print("Running Reconstruction Attack on Pretrained ERNIE 2.0 Embeddings")
    print("=" * 80)
    pretrained_results = run_reconstruction_attack(
        pretrained_test_embeddings, test_df['sequence'].tolist(),
        "pretrained_ernie", output_dir
    )

    print("\n" + "=" * 80)
    print("Running Reconstruction Attack on Fine-tuned ERNIE 2.0 Embeddings")
    print("=" * 80)
    finetuned_results = run_reconstruction_attack(
        finetuned_test_embeddings, test_df['sequence'].tolist(),
        "finetuned_ernie", output_dir
    )
    ...
    print(f"\nAverage Error-Based Privacy Gain: {avg_display_gain:.2f}%")
    ...
    print("\n" + "=" * 80)
    print("ERNIE 2.0 PRIVACY-UTILITY ANALYSIS SUMMARY")
    print("=" * 80)
    print(f"Model: ERNIE 2.0 ({model_name})")
    print(f"Dataset: HS3D (Splice Site Prediction)")
    print(f"Architecture: 12 layers, 768 dimensions, 110M parameters")
    ...
    plt.title('ERNIE 2.0 Fine-tuning Privacy-Utility Analysis: Summary Results', fontsize=14, pad=20)
    plt.tight_layout()
