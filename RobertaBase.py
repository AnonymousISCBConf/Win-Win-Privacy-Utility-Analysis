...
output_dir = "./roberta-base-result-double-win/"
...
def finetune_roberta(tokenized_train, tokenized_test, tokenizer):
    print("Starting fine-tuning of RoBERTa-base...")

    # Load pre-trained RoBERTa for sequence classification
    model = RobertaForSequenceClassification.from_pretrained(model_name, num_labels=2)

    # Define training arguments
    # RoBERTa-base has ~125M parameters (12 layers, 768 hidden dimensions)
    training_args = TrainingArguments(
        output_dir=f"{output_dir}finetune",
        learning_rate=2e-5,
        per_device_train_batch_size=32,  # Can use larger batch size than BERT-Large
        per_device_eval_batch_size=32,
        num_train_epochs=3,
        weight_decay=0.01,
        gradient_accumulation_steps=1,  # No need for gradient accumulation with base model
    ...
    print(f"Fine-tuned RoBERTa-base evaluation: {eval_result}")

    # Save the model
    model.save_pretrained(f"{output_dir}finetune_roberta_base")
    tokenizer.save_pretrained(f"{output_dir}finetune_roberta_base")

    return model, eval_result

# 4. Extract CLS token embeddings from RoBERTa
def extract_embeddings(model, tokenizer, sequences):
    print(f"Extracting embeddings from RoBERTa-base model...")
    ...
    all_embeddings = []
    batch_size = 32  # Can use larger batch size for base model
    ...
    print(f"Extracted embeddings shape: {embeddings.shape}")  # Should be (n_samples, 768)
...
def train_mlp(train_embeddings, train_labels, test_embeddings, test_labels, model_type):
    ...
    # Initialize model
    input_dim = train_embeddings.shape[1]  # 768 for RoBERTa-base
...
def create_comprehensive_visualizations(all_metrics):
    ...
    plt.title('RoBERTa-base Training Metrics by Epoch', fontsize=14)
    ...
    plt.title('RoBERTa-base Performance Metrics by Epoch', fontsize=14, pad=20)
    plt.tight_layout()
...
def run_reconstruction_attack(embeddings, sequences, model_name, output_dir="./roberta-base-result-double-win/"):
...
def compare_reconstruction_attacks(pretrained_results, finetuned_results, output_dir="./roberta-base-result-double-win/"):
...
def main():
    print("Starting RoBERTa-base fine-tuning, embedding extraction, and privacy analysis pipeline")
    ...
    print("Loading pretrained RoBERTa-base model...")
    ...
    _, pretrained_metrics = train_mlp(
        ...
        model_type="pretrained_roberta_base"
    )
    ...
    _, finetuned_metrics = train_mlp(
        ...
        model_type="finetuned_roberta_base"
    )
    ...
    plt.title('RoBERTa-base Fine-tuning Privacy-Utility Analysis: Summary Results', fontsize=14, pad=20)
    plt.tight_layout()
