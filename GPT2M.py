...
output_dir = "./gpt2-medium-result-double-win/"
...
# 3. Fine-tuning GPT-2 Medium
def finetune_gpt2(tokenized_train, tokenized_test, tokenizer):
    print("Starting fine-tuning of GPT-2 Medium...")

    # Load pre-trained GPT-2 Medium for sequence classification
    model = GPT2ForSequenceClassification.from_pretrained(model_name, num_labels=2)
    
    # GPT-2 needs pad_token_id to be set
    model.config.pad_token_id = tokenizer.pad_token_id
    
    # Resize token embeddings to account for new tokens
    model.resize_token_embeddings(len(tokenizer))

    # Define training arguments - adjusted for Medium model
    training_args = TrainingArguments(
        output_dir=f"{output_dir}finetune",
        learning_rate=3e-5,  # Slightly lower for larger model
        per_device_train_batch_size=16,  # Reduced batch size for Medium model
    ...
    print(f"Fine-tuned GPT-2 Medium evaluation: {eval_result}")

    # Save the model
    model.save_pretrained(f"{output_dir}finetune_gpt2_medium")
    tokenizer.save_pretrained(f"{output_dir}finetune_gpt2_medium")

    return model, eval_result

# 4. Extract LAST token embeddings from GPT-2 (autoregressive model)
def extract_embeddings(model, tokenizer, sequences):
    print(f"Extracting embeddings from GPT-2 Medium model...")

    # Use the base GPT-2 model for feature extraction
    feature_extractor = model.transformer
    feature_extractor.eval()

    all_embeddings = []
    batch_size = 16  # Reduced batch size for GPT-2 Medium
    ...
    with torch.no_grad():
        for i in range(0, len(sequences), batch_size):
            ...
            # GPT-2: Extract from LAST token position (autoregressive)
...
def train_mlp(train_embeddings, train_labels, test_embeddings, test_labels, model_type):
    ...
    # Initialize model
    input_dim = train_embeddings.shape[1]  # Embedding dimension (1024 for GPT-2 Medium)
...
def create_comprehensive_visualizations(all_metrics):
    ...
    plt.title('GPT-2 Medium Training Metrics by Epoch', fontsize=14)
    ...
    plt.title('GPT-2 Medium Performance Metrics by Epoch', fontsize=14, pad=20)
    plt.tight_layout()
...
def run_reconstruction_attack(embeddings, sequences, model_name, output_dir="./gpt2-medium-result-double-win/"):
...
def compare_reconstruction_attacks(pretrained_results, finetuned_results, output_dir="./gpt2-medium-result-double-win/"):
...
def main():
    print("Starting GPT-2 Medium fine-tuning, embedding extraction, and privacy analysis pipeline")
    print(f"Using GPT-2 model: {model_name}")
    ...
    print("Loading pretrained GPT-2 Medium model...")
    ...
    _, pretrained_metrics = train_mlp(
        ...
        model_type="pretrained_gpt2_medium"
    )
    ...
    # Fine-tune the model
    finetuned_model, gpt2_eval_results = finetune_gpt2(tokenized_train, tokenized_test, tokenizer)
    ...
    _, finetuned_metrics = train_mlp(
        ...
        model_type="finetuned_gpt2_medium"
    )
    ...
    plt.title('GPT-2 Medium Privacy-Utility Analysis: Summary Results', fontsize=14, pad=20)
    plt.tight_layout()
