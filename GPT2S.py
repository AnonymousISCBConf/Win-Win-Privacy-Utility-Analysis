...
# Use GPT-2 Small
model_name = "gpt2"
output_dir = "./gpt2-small-result-double-win/"
...
def finetune_gpt2(tokenized_train, tokenized_test, tokenizer):
    print("Starting fine-tuning of GPT-2 Small...")
    ...
    training_args = TrainingArguments(
        ...
        per_device_train_batch_size=32,  # Larger batch size for smaller model
    ...
    print(f"Fine-tuned GPT-2 Small evaluation: {eval_result}")

    # Save the model
    model.save_pretrained(f"{output_dir}finetune_gpt2_small")
    tokenizer.save_pretrained(f"{output_dir}finetune_gpt2_small")

    return model, eval_result

# 4. Extract LAST token embeddings from GPT-2 (autoregressive model)
def extract_embeddings(model, tokenizer, sequences):
    print(f"Extracting embeddings from GPT-2 Small model...")
    ...
    all_embeddings = []
    batch_size = 32  # Larger batch size for GPT-2 Small
...
def train_mlp(train_embeddings, train_labels, test_embeddings, test_labels, model_type):
    ...
    # Initialize model
    input_dim = train_embeddings.shape[1]  # Embedding dimension (768 for GPT-2 Small)
...
def create_comprehensive_visualizations(all_metrics):
    ...
    plt.title('GPT-2 Small Training Metrics by Epoch', fontsize=14)
    ...
    plt.title('GPT-2 Small Performance Metrics by Epoch', fontsize=14, pad=20)
    plt.tight_layout()
...
def run_reconstruction_attack(embeddings, sequences, model_name, output_dir="./gpt2-small-result-double-win/"):
...
def compare_reconstruction_attacks(pretrained_results, finetuned_results, output_dir="./gpt2-small-result-double-win/"):
...
def main():
    print("Starting GPT-2 Small fine-tuning, embedding extraction, and privacy analysis pipeline")
    ...
    print("Loading pretrained GPT-2 Small model...")
    ...
    _, pretrained_metrics = train_mlp(
        ...
        model_type="pretrained_gpt2_small"
    )
    ...
    _, finetuned_metrics = train_mlp(
        ...
        model_type="finetuned_gpt2_small"
    )
    ...
    plt.title('GPT-2 Small Privacy-Utility Analysis: Summary Results', fontsize=14, pad=20)
    plt.tight_layout()
