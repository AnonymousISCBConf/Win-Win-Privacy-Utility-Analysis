...
# Use BERT large cased (changed from bert-base-cased)
model_name = "bert-large-cased"
output_dir = "./bert-large-result-double-win/"
...
def finetune_bert(tokenized_train, tokenized_test, tokenizer):
    print("Starting fine-tuning of BERT-Large...")
    ...
    # Note: BERT-Large has more parameters, so we may need to adjust batch size
    training_args = TrainingArguments(
        output_dir=f"{output_dir}finetune",
        learning_rate=2e-5,  # Slightly lower learning rate for BERT-Large
    ...
    print(f"Fine-tuned BERT-Large evaluation: {eval_result}")

    # Save the model
    model.save_pretrained(f"{output_dir}finetune_bert_large")
    tokenizer.save_pretrained(f"{output_dir}finetune_bert_large")

    return model, eval_result

# 4. Extract CLS token embeddings from BERT
def extract_embeddings(model, tokenizer, sequences):
    print(f"Extracting embeddings from BERT-Large model...")
    ...
    all_embeddings = []
    batch_size = 16  # Reduced batch size for BERT-Large
...
def train_mlp(train_embeddings, train_labels, test_embeddings, test_labels, model_type):
    ...
    # Initialize model
    input_dim = train_embeddings.shape[1]  # Embedding dimension (1024 for BERT-Large)
...
def create_comprehensive_visualizations(all_metrics):
    ...
    plt.title('BERT-Large Training Metrics by Epoch', fontsize=14)
    ...
    plt.title('BERT-Large Performance Metrics by Epoch', fontsize=14, pad=20)
    plt.tight_layout()
...
def run_reconstruction_attack(embeddings, sequences, model_name, output_dir="./bert-large-result-double-win/"):
...
def compare_reconstruction_attacks(pretrained_results, finetuned_results, output_dir="./bert-large-result-double-win/"):
...
# 18. Visualize Privacy Gain and PDS - Modified for log values
...
# 19. Create Pareto Frontier Plots - For both metrics
...
def main():
    print("Starting BERT-Large fine-tuning, embedding extraction, and privacy analysis pipeline")
    ...
    print("Loading pretrained BERT-Large model...")
    ...
    _, pretrained_metrics = train_mlp(
        ...
        model_type="pretrained_bert_large"
    )
    ...
    _, finetuned_metrics = train_mlp(
        ...
        model_type="finetuned_bert_large"
    )
    ...
    # Calculate Error-Based Privacy Gain
    ...
    # Calculate Pareto Dominance Scores using both metrics
    ...
    # Visualize advanced privacy metrics
    ...
    # Create Pareto frontier plots for both metrics
    ...
    plt.title('BERT-Large Fine-tuning Privacy-Utility Analysis: Summary Results', fontsize=14, pad=20)
    plt.tight_layout()
