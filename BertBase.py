...
# Use BERT base cased (as Pan et al. used)
model_name = "bert-base-cased"
output_dir = "./bert-result-double-win/"
...
def create_datasets(train_df, test_df):
    ...
    def tokenize_function(examples):
        # For DNA sequences, it might be better to tokenize each nucleotide separately
        tokenized = []
...
# 3. Fine-tuning BERT
def finetune_bert(tokenized_train, tokenized_test, tokenizer):
    print("Starting fine-tuning of BERT...")
    ...
    training_args = TrainingArguments(
        ...
        learning_rate=3e-5,  # BERT typically uses slightly higher learning rate than XLNet
        per_device_train_batch_size=32,  # BERT can handle larger batch sizes
    ...
    print(f"Fine-tuned BERT evaluation: {eval_result}")

    # Save the model
    model.save_pretrained(f"{output_dir}finetune_bert")
    tokenizer.save_pretrained(f"{output_dir}finetune_bert")

    return model, eval_result

# 4. Extract CLS token embeddings from BERT
def extract_embeddings(model, tokenizer, sequences):
    print(f"Extracting embeddings from BERT model...")

    # Use the base BERT model for feature extraction
    ...
    with torch.no_grad():
        for i in range(0, len(sequences), batch_size):
            ...
            # Extract CLS token embeddings for BERT (first token)
            # outputs[0] contains the last hidden states
            cls_token_embeddings = outputs[0][:, 0, :].cpu().numpy()
            all_embeddings.append(cls_token_embeddings)
...
def train_mlp(train_embeddings, train_labels, test_embeddings, test_labels, model_type):
    ...
    # Initialize model
    input_dim = train_embeddings.shape[1]  # Embedding dimension
    ...
    # Extract and return all metrics for comprehensive reporting
...
def create_comprehensive_visualizations(all_metrics):
    # Convert metrics to arrays for easier processing
    ...
    # Calculate average performance and standard deviations
    ...
    plt.title('BERT Training Metrics by Epoch', fontsize=14)
    ...
    # Create a table visualization of all metrics for reporting
    plt.figure(figsize=(12, 5))
    ...
    for i in range(len(table_data)):
        for j in range(len(table_data[0])):
            ...
            elif j == 0:  # Epoch column
                cell.set_facecolor('#D9E1F2')

    plt.title('BERT Performance Metrics by Epoch', fontsize=14, pad=20)
    plt.tight_layout()
...
def visualize_embeddings_pca(embeddings, labels, dataset_name):
    ...
    # For large embeddings, sample to speed up computation
...
class ReconstructionAttackModel(nn.Module):
    ...
    def forward(self, x):
        x = self.bn1(self.sigmoid(self.fc1(x)))
        x = self.bn2(self.sigmoid(self.fc2(x)))
        x = self.fc3(x)  # No activation for final layer (will use CrossEntropyLoss)
        return x
...
def run_reconstruction_attack(embeddings, sequences, model_name, output_dir="./bert-result-double-win/"):
    ...
    # Verify sequences are at least seq_length
    ...
    # Store accuracy for each position
    position_accuracy = []
    confusion_matrices = []
    ...
    for pos in range(seq_length):
        ...
        # Create positional embeddings for this position
        pos_embeddings = torch.stack([create_positional_embedding(pos, embedding_dim)
                                    for _ in range(len(embeddings))])

        # Extract true nucleotide at this position
        y_true = torch.tensor([nuc_to_int.get(seq[pos].upper(), 0) for seq in sequences])

        # Create input data (concatenated embedding and positional embedding)
        ...
        # Create confusion matrix for this position
    ...
    # Plot accuracy by position (similar to Pan's Fig. 3)
    plt.figure(figsize=(10, 6))
    ...
    # Plot heatmap of all confusion matrices
    plt.figure(figsize=(15, 10))
    # Create a 5x4 grid of confusion matrices
    nrows, ncols = 5, 4
    ...
    # Calculate and display class-wise performance
    class_accuracy = {}
    for nuc in ['A', 'C', 'G', 'T']:
        ...
        for pos in range(seq_length):
            ...
            if cm[nuc_idx].sum() > 0:  # Avoid division by zero
...
def compare_reconstruction_attacks(pretrained_results, finetuned_results, output_dir="./bert-result-double-win/"):
    ...
    # Shade regions where fine-tuned is more private
    ...
    # Compare nucleotide-wise accuracy
    ...
    # Create a pretty table visualization
...
# 13. NEW: Calculate Error-Based Privacy Gain with true logarithmic transformation
def calculate_privacy_gain_log(pretrained_acc, finetuned_acc):
    ...
    for i in range(len(pretrained_error)):
        if pretrained_error[i] > 0 and finetuned_error[i] > 0:
            # True logarithmic transformation with interpretable cap
            ratio = finetuned_error[i] / pretrained_error[i]
            # Cap ratio to 1.5x (50% improvement) for visualization purposes
            ratio = min(ratio, 1.5)  
            privacy_gain[i] = np.log(ratio)
        elif finetuned_error[i] > pretrained_error[i]:
            privacy_gain[i] = np.log(1.5)  # Use the cap value for extreme cases
        else:
            privacy_gain[i] = 0
    
    return privacy_gain

# 14. NEW: Calculate Pareto Dominance Score based on delta privacy (PDS-Delta)
def calculate_pds_delta(delta_privacy, utility_delta):
    ...
    for i in range(len(delta_privacy)):
        if delta_privacy[i] > 0 and utility_delta > 0:
            ...
            pds[i] = -2  # Lose-Lose
        else:  # delta_privacy[i] < 0 or utility_delta < 0
            pds[i] = -1  # Tradeoff
            
    return pds

# 15. NEW: Calculate Pareto Dominance Score based on privacy gain (PDS-Gain)
def calculate_pds_gain(privacy_gain, utility_delta):
    ...
    Uses different thresholds to distinguish from delta_privacy approach
    """
    pds = np.zeros(len(privacy_gain))
    
    # Define significant privacy gain threshold (log(1.1) ˜ 0.095 = ~10% improvement)
    significant_gain = 0.095
    # Define modest privacy gain threshold (log(1.05) ˜ 0.049 = ~5% improvement)
    modest_gain = 0.049
    
    for i in range(len(privacy_gain)):
        # Strong Win-Win: Significant privacy gain + utility gain
        if privacy_gain[i] > significant_gain and utility_delta > 0:
            pds[i] = 2
        # Privacy-focused Win: Significant privacy gain with neutral utility
        elif privacy_gain[i] > significant_gain and utility_delta == 0:
            pds[i] = 1
        # Utility-focused Win: Modest privacy gain with utility gain
        elif privacy_gain[i] > modest_gain and utility_delta > 0:
            pds[i] = 1
        # Neutral: Minor privacy changes or balanced tradeoffs
        elif (modest_gain >= privacy_gain[i] >= 0 and utility_delta >= 0) or (privacy_gain[i] == 0 and utility_delta == 0):
            pds[i] = 0
        # Lose-Lose: Privacy loss and utility loss
        elif privacy_gain[i] < 0 and utility_delta < 0:
            pds[i] = -2
        # Tradeoff: Sacrificing privacy for utility or vice versa
        else:
            pds[i] = -1
    
    return pds

# 16. NEW: Calculate Win-Win Percentage (WWP)
...
# 17. NEW: Statistical Significance Testing
def statistical_significance_test(privacy_gain):
    ...
    # Two-tailed test with n-1 degrees of freedom
    p_value = 2 * (1 - stats.t.cdf(abs(t_stat), len(privacy_gain)-1))
    
    return t_stat, p_value, mean_pg, se_pg

# 18. NEW: Visualize Privacy Gain and PDS - Modified for log values
def visualize_privacy_metrics(delta_privacy, privacy_gain, pds_delta, pds_gain, utility_delta, output_dir):
    ...
    # 1. Plot Privacy Gain - Convert log values to percentage for display
    plt.figure(figsize=(10, 6))
    ...
    # 6. Create summary box with WWP and other metrics
    wwp_delta = calculate_wwp(pds_delta)
    wwp_gain = calculate_wwp(pds_gain)
    
    # Add a textbox summarizing the results
    ...
    summary_text = [
        ...
        f"Average Privacy Gain: {avg_display:.1f}%",  # Use converted value
    ...
    return {
        ...
        "avg_privacy_gain": avg_display,  # Return converted percentage
...
# 19. NEW: Create Pareto Frontier Plots - For both metrics
def create_position_pareto_frontiers(delta_privacy, privacy_gain, utility_delta, output_dir):
    """
    Create Pareto frontier plots for positions using both delta_privacy and privacy_gain metrics
    """
    seq_length = len(delta_privacy)
    positions = np.arange(1, seq_length + 1)
    
    # 1. Delta Privacy Pareto Frontier
    # Categorize positions based on delta_privacy
    ...
    # Create scatter plot for delta_privacy
    ...
    # Find top positions by delta_privacy
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}position_pareto_frontier_delta.png", dpi=300)
    plt.close()
    
    # 2. Privacy Gain Pareto Frontier
    # Convert log values to percentage for display
    display_values = (np.exp(privacy_gain) - 1) * 100
    display_values = np.minimum(display_values, 50.0)
    
    # Categorize positions based on privacy_gain
    ...
    # Create scatter plot for privacy_gain
    plt.figure(figsize=(12, 8))
    ...
    # Find top positions by privacy_gain
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}position_pareto_frontier_gain.png", dpi=300)
    plt.close()
    
    # Create tables showing position rankings for both metrics
    
    # Sort positions by delta_privacy
    sorted_delta_indices = np.argsort(delta_privacy)[::-1]  # Descending order
    sorted_delta_positions = positions[sorted_delta_indices]
    sorted_delta_values = delta_privacy[sorted_delta_indices] * 100  # Convert to percentage
    
    # Sort positions by privacy_gain
    sorted_gain_indices = np.argsort(privacy_gain)[::-1]  # Descending order
    sorted_gain_positions = positions[sorted_gain_indices]
    sorted_gain_values = (np.exp(privacy_gain[sorted_gain_indices]) - 1) * 100  # Convert to percentage
    sorted_gain_values = np.minimum(sorted_gain_values, 50.0)  # Apply cap
    ...
    # Add top 20 positions from each metric
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}position_ranking_comparison.png", dpi=300)
    plt.close()
    
    # Create a summary of win-win positions from both perspectives
    ...
    # Find positions that are win-win in both metrics
    common_win_win = set(win_win_delta_positions).intersection(set(win_win_gain_positions))
    only_delta_win_win = set(win_win_delta_positions) - common_win_win
    only_gain_win_win = set(win_win_gain_positions) - common_win_win
    
    # Create a visualization comparing the win-win positions
    plt.figure(figsize=(10, 6))
    
    # Create a Venn diagram-style visualization
    from matplotlib_venn import venn2
    
    v = venn2([set(win_win_delta_positions), set(win_win_gain_positions)], 
             ('Win-Win (Delta Privacy)', 'Win-Win (Privacy Gain)'))
    
    # Add counts to the diagram
    v.get_label_by_id('10').set_text(f'Only Delta\n{len(only_delta_win_win)}')
    v.get_label_by_id('01').set_text(f'Only Gain\n{len(only_gain_win_win)}')
    v.get_label_by_id('11').set_text(f'Both\n{len(common_win_win)}')
    
    plt.title('Comparison of Win-Win Positions Between Metrics', fontsize=14)
    plt.tight_layout()
    plt.savefig(f"{output_dir}win_win_comparison.png", dpi=300)
    plt.close()
...
# Example: Create a side-by-side comparison showing differences
def create_pds_comparison(pds_delta, pds_gain, delta_privacy, privacy_gain, output_dir):
    ...
    # 1. Side-by-side comparison with difference highlighting
    ...
    # Plot 3: Difference map (where they disagree)
    ...
    # Annotate positions where they differ
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}pds_detailed_comparison.png", dpi=300)
    plt.close()
    
    # 2. Create a heatmap showing both metrics together
    fig, ax = plt.subplots(figsize=(14, 6))
    
    # Create a matrix with both PDS values
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}pds_heatmap_comparison.png", dpi=300)
    plt.close()
    
    # 3. Create a summary statistics table
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.axis('off')
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}pds_comparison_summary.png", dpi=300)
    plt.close()
    
    print(f"Created enhanced comparison visualizations in {output_dir}")
    print(f"Positions where PDS-Delta and PDS-Gain disagree: {list(disagreement_positions)}")
    print(f"  PDS-Delta has {win_win_delta} Win-Win positions")
    print(f"  PDS-Gain has {win_win_gain} Win-Win positions")
    
    return disagreement_positions

# Example usage (add this to your main() function):
# disagreement_positions = create_pds_comparison(pds_delta, pds_gain, delta_privacy, privacy_gain, output_dir)
# 20. Main execution function
def main():
    print("Starting BERT fine-tuning, embedding extraction, and privacy analysis pipeline")
    ...
    # Load pretrained BERT model for baseline embeddings
    ...
    # Train MLP classifier on pretrained embeddings to get baseline utility
    ...
    # Fine-tune the model
    finetuned_model, bert_eval_results = finetune_bert(tokenized_train, tokenized_test, tokenizer)
    ...
    # Train MLP classifier on fine-tuned embeddings
    ...
    # Run reconstruction attack on pretrained embeddings
    ...
    # Run reconstruction attack on fine-tuned embeddings
    ...
    # Compare reconstruction attack results
    delta_privacy, pretrained_acc, finetuned_acc = compare_reconstruction_attacks(pretrained_results, finetuned_results, output_dir)

    # NEW: Calculate Error-Based Privacy Gain
    ...
    # NEW: Calculate Pareto Dominance Scores using both metrics
    pds_delta = calculate_pds_delta(delta_privacy, utility_delta)
    pds_gain = calculate_pds_gain(privacy_gain, utility_delta)
    # For debugging
     # ?? ADD DEBUGGING CODE HERE ??
    print("\n=== PDS Metric Comparison ===")
    print(f"Positions where PDS-Delta and PDS-Gain differ:")
    diff_positions = np.where(pds_delta != pds_gain)[0] + 1
    print(f"  Positions: {list(diff_positions)}")
    print(f"  Total differing: {len(diff_positions)} out of {len(pds_delta)}")
    
    for pos in diff_positions:
        idx = pos - 1
        label_map = {-2: 'Lose-Lose', -1: 'Tradeoff', 0: 'Neutral', 1: 'Privacy Gain', 2: 'Win-Win'}
        print(f"\nPosition {pos}:")
        print(f"  delta_privacy: {delta_privacy[idx]:.4f} ({delta_privacy[idx]*100:.2f}%)")
        print(f"  privacy_gain: {privacy_gain[idx]:.4f} (log scale) = {(np.exp(privacy_gain[idx])-1)*100:.2f}% improvement")
        print(f"  PDS-Delta: {pds_delta[idx]:.0f} ? {label_map[pds_delta[idx]]}")
        print(f"  PDS-Gain: {pds_gain[idx]:.0f} ? {label_map[pds_gain[idx]]}")
    # ?? END DEBUGGING CODE ??
    print("\n=== Debugging PDS Calculations ===")
    print("Sample delta_privacy:", delta_privacy[:5])
    print("Sample privacy_gain:", privacy_gain[:5])
    print("Sample pds_delta:", pds_delta[:5])
    print("Sample pds_gain:", pds_gain[:5])
    ...
    diff_indices = np.where(pds_delta != pds_gain)[0]
    print(f"Positions where PDS metrics differ: {diff_indices}")
    if len(diff_indices) > 0:
       print(f"Example at position {diff_indices[0]}:")
       pos = diff_indices[0]
       print(f"  delta_privacy: {delta_privacy[pos]}, pds_delta: {pds_delta[pos]}")
       print(f"  privacy_gain: {privacy_gain[pos]}, pds_gain: {pds_gain[pos]}")
       
    # Calculate Win-Win Percentages for both metrics
    wwp_delta = calculate_wwp(pds_delta)
    wwp_gain = calculate_wwp(pds_gain)
    ...
    print(f"Average Pareto Dominance Score (Delta-Privacy): {np.mean(pds_delta):.2f}")
    print(f"Average Pareto Dominance Score (Privacy-Gain): {np.mean(pds_gain):.2f}")
    ...
    # NEW: Visualize advanced privacy metrics
    ...
    # NEW: Create Pareto frontier plots for both metrics
    position_analysis = create_position_pareto_frontiers(delta_privacy, privacy_gain, utility_delta, output_dir)
    
    # Report win-win positions for both metrics
    ...
    print("\nPrivacy Metrics:")
    print(f"  Pretrained Reconstruction Accuracy: {np.mean(pretrained_acc):.4f}")
    print(f"  Fine-tuned Reconstruction Accuracy: {np.mean(finetuned_acc):.4f}")
    print(f"  Delta Privacy:                      {np.mean(delta_privacy):.4f}")
    print(f"  Error-Based Privacy Gain:           {avg_display_gain:.2f}%")
    
    print("\nJoint Metrics (Delta-Privacy):")
    print(f"  Pareto Dominance Score: {np.mean(pds_delta):.2f}")
    print(f"  Win-Win Percentage:     {wwp_delta:.2f}%")
    
    print("\nJoint Metrics (Privacy-Gain):")
    print(f"  Pareto Dominance Score: {np.mean(pds_gain):.2f}")
    print(f"  Win-Win Percentage:     {wwp_gain:.2f}%")
    print(f"  Privacy Statistical Significance: t={t_stat:.2f}, p={p_value:.4f} ({significance})")
    
    # Create a comprehensive summary table with both metrics
    summary_data = {
        'Metric': [
            'Pretrained Accuracy',
            'Fine-tuned Accuracy',
            ...
            'Delta Privacy',
            'Privacy Gain',
            'Win-Win Percentage (Delta)',
            'Win-Win Percentage (Gain)',
            'Average PDS (Delta)',
            'Average PDS (Gain)',
            'Privacy t-statistic',
            'Privacy p-value'
        ...
        'Interpretation': [
            ...
            "Fine-tuned task performance",
            "Increase in model utility",
            "Baseline privacy vulnerability",
            "Fine-tuned privacy vulnerability",
            "Average change in reconstruction accuracy",
            "Percentage increase in reconstruction difficulty",
            "Percentage of positions with delta privacy & utility gain",
            "Percentage of positions with privacy & utility gain",
            "Delta privacy-utility balance (-2 to +2 scale)",
            "Privacy gain-utility balance (-2 to +2 scale)",
            "Measure of statistical significance",
            f"Privacy improvement is {significance}"
        ]
    }
    ...
    # Create a pretty summary table visualization
    plt.figure(figsize=(12, 6))
    plt.axis('off')
    ...
    # Highlight win-win rows if applicable
    ...
    if is_win_win_delta:
        for j in range(len(summary_df.columns)):
            cell = table[(7, j)]  # Win-Win Percentage (Delta) row
            cell.set_facecolor('#E2EFDA')
            cell.set_text_props(fontweight='bold')
            
    if is_win_win_gain:
        for j in range(len(summary_df.columns)):
            cell = table[(8, j)]  # Win-Win Percentage (Gain) row
            cell.set_facecolor('#E2EFDA')
            cell.set_text_props(fontweight='bold')
    
    plt.title('BERT Fine-tuning Privacy-Utility Analysis: Summary Results', fontsize=14, pad=20)
    plt.tight_layout()
