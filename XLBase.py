...
output_dir = "./xlnet-base-result-double-win/"
...
def finetune_xlnet(tokenized_train, tokenized_test, tokenizer):
    print("Starting fine-tuning of XLNet-Base...")
    ...
    print(f"Fine-tuned XLNet-Base evaluation: {eval_result}")

    # Save the model
    model.save_pretrained(f"{output_dir}finetune_xlnet_base")
    tokenizer.save_pretrained(f"{output_dir}finetune_xlnet_base")

    return model, eval_result

# 4. Extract LAST token embeddings from XLNet (key difference from BERT)
def extract_embeddings(model, tokenizer, sequences):
    print(f"Extracting embeddings from XLNet-Base model...")
...
def train_mlp(train_embeddings, train_labels, test_embeddings, test_labels, model_type):
    ...
    # Initialize model
    input_dim = train_embeddings.shape[1]  # Embedding dimension (768 for XLNet-Base)
...
def create_comprehensive_visualizations(all_metrics):
    ...
    plt.title('XLNet-Base Training Metrics by Epoch', fontsize=14)
    ...
    plt.title('XLNet-Base Performance Metrics by Epoch', fontsize=14, pad=20)
    plt.tight_layout()
...
def run_reconstruction_attack(embeddings, sequences, model_name, output_dir="./xlnet-base-result-double-win/"):
...
def compare_reconstruction_attacks(pretrained_results, finetuned_results, output_dir="./xlnet-base-result-double-win/"):
...
def main():
    print("Starting XLNet-Base fine-tuning, embedding extraction, and privacy analysis pipeline")
    ...
    print("Loading pretrained XLNet-Base model...")
    ...
    _, pretrained_metrics = train_mlp(
        ...
        model_type="pretrained_xlnet_base"
    )
    ...
    _, finetuned_metrics = train_mlp(
        ...
        model_type="finetuned_xlnet_base"
    )
    ...
    plt.title('XLNet-Base Privacy-Utility Analysis: Summary Results', fontsize=14, pad=20)
    plt.tight_layout()
