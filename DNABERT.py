...
# Use DNABERT-6 (changed from bert-large-cased)
model_name = "zhihan1996/DNA_bert_6"
output_dir = "./dnabert-6-result-double-win/"
...
def load_data():
    ...
    # Convert sequences to strings and uppercase (DNABERT requirement)
    train_df['sequence'] = train_df['sequence'].astype(str).str.upper()
    test_df['sequence'] = test_df['sequence'].astype(str).str.upper()
    
    # Filter out sequences with ambiguous nucleotides (N)
...
def create_datasets(train_df, test_df):
    ...
    # Load DNABERT tokenizer - handles k-mer conversion automatically
    tokenizer = AutoTokenizer.from_pretrained(model_name, trust_remote_code=True)

    def tokenize_function(examples):
        # DNABERT tokenizer handles k-mer conversion internally
        # Pass raw DNA sequences directly (no spacing needed)
        return tokenizer(
            examples['sequence'],
            padding='max_length',
            truncation=True,
            max_length=100,  # k-mer tokens (adjust based on your sequences)
            return_tensors='pt'
        )
...
# 3. Fine-tuning DNABERT
def finetune_dnabert(tokenized_train, tokenized_test, tokenizer):
    print("Starting fine-tuning of DNABERT-6...")

    # Load pre-trained DNABERT for sequence classification
    ...
    # Define training arguments - DNABERT recommendations
    training_args = TrainingArguments(
        output_dir=f"{output_dir}finetune",
        learning_rate=2e-4,  # DNABERT uses 2e-4
        per_device_train_batch_size=32,  # DNABERT uses 32
        per_device_eval_batch_size=32,
        num_train_epochs=5,  # DNABERT typically 3-5 epochs
        weight_decay=0.01,
    ...
    print(f"Fine-tuned DNABERT-6 evaluation: {eval_result}")

    # Save the model
    model.save_pretrained(f"{output_dir}finetune_dnabert6")
    tokenizer.save_pretrained(f"{output_dir}finetune_dnabert6")

    return model, eval_result

# 4. Extract CLS token embeddings from DNABERT
def extract_embeddings(model, tokenizer, sequences):
    print(f"Extracting embeddings from DNABERT-6 model...")
    ...
    all_embeddings = []
    batch_size = 32  # DNABERT can handle 32

    with torch.no_grad():
        for i in range(0, len(sequences), batch_size):
            batch_sequences = sequences[i:i+batch_size]

            # Direct tokenization - DNABERT handles k-mer conversion
            inputs = tokenizer(
                batch_sequences,
                padding='max_length',
                truncation=True,
...
def train_mlp(train_embeddings, train_labels, test_embeddings, test_labels, model_type):
    ...
    # Initialize model
    input_dim = train_embeddings.shape[1]  # Embedding dimension (768 for DNABERT-6)
...
def create_comprehensive_visualizations(all_metrics):
    ...
    plt.title('DNABERT-6 Training Metrics by Epoch', fontsize=14)
    ...
    plt.title('DNABERT-6 Performance Metrics by Epoch', fontsize=14, pad=20)
    plt.tight_layout()
...
def run_reconstruction_attack(embeddings, sequences, model_name, output_dir="./dnabert-6-result-double-win/"):
...
def compare_reconstruction_attacks(pretrained_results, finetuned_results, output_dir="./dnabert-6-result-double-win/"):
...
def create_position_pareto_frontiers(delta_privacy, privacy_gain, utility_delta, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}attack_results/win_win_comparison.png", dpi=300)
    plt.close()
...
def main():
    print("Starting DNABERT-6 fine-tuning, embedding extraction, and privacy analysis pipeline")
    print(f"Using DNABERT model: {model_name}")
    ...
    # Load pretrained DNABERT model for baseline embeddings
    print("Loading pretrained DNABERT-6 model...")
    ...
    _, pretrained_metrics = train_mlp(
        ...
        model_type="pretrained_dnabert6"
    )
    ...
    # Fine-tune the model
    finetuned_model, bert_eval_results = finetune_dnabert(tokenized_train, tokenized_test, tokenizer)
    ...
    # PCA visualizations
    visualize_embeddings_pca(pretrained_test_embeddings, test_df['label'].values, "Pretrained")
    visualize_embeddings_pca(finetuned_test_embeddings, test_df['label'].values, "Finetuned")
    ...
    print("\n==== Results Summary ====")
    print(f"Model: DNABERT-6 (k=6, 768 hidden units, ~86M parameters)")
    ...
    plt.title('DNABERT-6 Fine-tuning Privacy-Utility Analysis: Summary Results', fontsize=14, pad=20)
    plt.tight_layout()
