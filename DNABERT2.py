...
def disable_flash_attention():
    """More aggressive monkey-patch to disable FlashAttention"""
    try:
        # Patch 1: BERT attention
        import transformers.models.bert.modeling_bert as modeling_bert
        if hasattr(modeling_bert.BertSelfAttention, 'forward'):
            original_forward = modeling_bert.BertSelfAttention.forward
            def patched_forward(self, hidden_states, attention_mask=None, **kwargs):
                kwargs.pop('use_flash_attention_2', None)
                kwargs.pop('attn_implementation', None)
                return original_forward(self, hidden_states, attention_mask)
            modeling_bert.BertSelfAttention.forward = patched_forward
            print("? Patched BertSelfAttention")
    except Exception as e:
        print(f"Could not patch BERT: {e}")
    
    try:
        # Patch 2: Disable Triton's flash attention
        import torch.nn.functional as F
        original_dot = torch.Tensor.dot if hasattr(torch.Tensor, 'dot') else None
        if original_dot:
            def safe_dot(self, other):
                return torch.matmul(self, other)
            # Don't actually patch this - just noting for reference
            print("? PyTorch dot available")
    except Exception as e:
        print(f"PyTorch patch note: {e}")
    
    try:
        # Patch 3: Mock flash_attn module
        import sys
        from unittest.mock import MagicMock
        if 'flash_attn' not in sys.modules:
            sys.modules['flash_attn'] = MagicMock()
            print("? Mocked flash_attn module")
    except Exception as e:
        print(f"Could not mock flash_attn: {e}")
...
# Use DNABERT-2 (117M parameters, 12 layers, 768 hidden dims, 12 attention heads)
model_name = "zhihan1996/DNABERT-2-117M"
output_dir = "./dnabert2-result-double-win/"
...
def load_data():
    ...
    # Convert sequences to strings and uppercase
...
# 2. Create HuggingFace datasets for DNABERT-2
def create_datasets(train_df, test_df):
    train_dataset = Dataset.from_pandas(train_df)
    test_dataset = Dataset.from_pandas(test_df)

    # Load DNABERT-2 tokenizer - uses BPE tokenization (not k-mer)
    tokenizer = AutoTokenizer.from_pretrained(model_name, trust_remote_code=True)

    def tokenize_function(examples):
        # DNABERT-2 uses BPE tokenization - pass raw sequences directly
        # No k-mer spacing needed (unlike DNABERT-1)
        return tokenizer(
            examples['sequence'],
            padding='max_length',
            truncation=True,
            max_length=512,  # DNABERT-2 can handle longer sequences with ALiBi
            return_tensors='pt'
        )

    print("Tokenizing datasets with DNABERT-2 BPE tokenizer...")
...
def finetune_dnabert(tokenized_train, tokenized_test, tokenizer):
    print("Starting fine-tuning of DNABERT-2...")

    # Load pre-trained DNABERT-2 for sequence classification
    ...
    # Define training arguments - adjusted for DNABERT-2
    training_args = TrainingArguments(
        output_dir=f"{output_dir}finetune",
        learning_rate=3e-5,  # DNABERT-2 typically uses 3e-5 to 5e-5
    ...
    def compute_metrics(pred):
        ...
        if isinstance(predictions, tuple):
            predictions = predictions[0]  # Take the first element (logits)
    ...
    print(f"Fine-tuned DNABERT-2 evaluation: {eval_result}")

    # Save the model
    model.save_pretrained(f"{output_dir}finetune_dnabert2")
    tokenizer.save_pretrained(f"{output_dir}finetune_dnabert2")

    return model, eval_result

# 4. Extract CLS token embeddings from DNABERT-2
def extract_embeddings(model, tokenizer, sequences):
    print(f"Extracting embeddings from DNABERT-2 model...")

    # Use the base BERT model for feature extraction
    # DNABERT-2 uses 'bert' attribute for the base model
    ...
    with torch.no_grad():
        for i in range(0, len(sequences), batch_size):
            ...
            # DNABERT-2 has 768 hidden dimensions
            cls_token_embeddings = outputs[0][:, 0, :].cpu().numpy()
            all_embeddings.append(cls_token_embeddings)

    # Concatenate all batches
    embeddings = np.vstack(all_embeddings)
    print(f"Extracted embeddings shape: {embeddings.shape} (expected: [N, 768])")
...
def train_mlp(train_embeddings, train_labels, test_embeddings, test_labels, model_type):
    ...
    # Initialize model - input_dim should be 768 for DNABERT-2
...
def create_comprehensive_visualizations(all_metrics):
    ...
    plt.title('DNABERT-2 Training Metrics by Epoch', fontsize=14)
    ...
    plt.title('DNABERT-2 Performance Metrics by Epoch', fontsize=14, pad=20)
    plt.tight_layout()
...
def run_reconstruction_attack(embeddings, sequences, model_name, output_dir="./dnabert2-result-double-win/"):
...
def compare_reconstruction_attacks(pretrained_results, finetuned_results, output_dir="./dnabert2-result-double-win/"):
...
def main():
    print("Starting DNABERT-2 fine-tuning, embedding extraction, and privacy analysis pipeline")
    print(f"Using DNABERT-2 model: {model_name}")
    print(f"Model specifications: 117M parameters, 12 layers, 768 hidden dims, 12 attention heads, BPE tokenization")
    ...
    # Load pretrained DNABERT-2 model for baseline embeddings
    print("Loading pretrained DNABERT-2 model...")
    ...
    _, pretrained_metrics = train_mlp(
        ...
        model_type="pretrained_dnabert2"
    )
    ...
    print("\n==== Results Summary ====")
    print(f"Model: DNABERT-2 (117M parameters, 12 layers, 768 hidden dims, BPE tokenization)")
    ...
    plt.title('DNABERT-2 Fine-tuning Privacy-Utility Analysis: Summary Results', fontsize=14, pad=20)
    plt.tight_layout()
